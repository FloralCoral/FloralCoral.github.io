<?php
	...
	$myQuery= $mysqli->query("SELECT Parish, Coordinates, Parish_Case_Sum FROM heatMapData");
?>
...
<html lang="en">
  <head>
    ...
    <title>Display Covid Cases By Parish</title>
  </head>
  <body>
    ...
    <div>
    	<h1 id="pageTitle">Covid Cases By Parish</h1>
    </div>
    ...
    <script>
      ...
      <?php
		  ...
		  {
  		  	//get the parish, coordinates, and case amount , row by row, then display them to the map
  			$parishName=$rows['Parish'];
  			$coordinates=$rows['Coordinates'];
        $caseAmount=$rows['Parish_Case_Sum'];
        
          //add the marker to the map, then append the message containing the info to the marker
  			echo "var marker = L.marker([$coordinates],).addTo(mymap);";
  			echo "marker.bindPopup(\"<b>$parishName</br>$caseAmount cases</b>\").openPopup();";
		  }
		  //free the memory
		  $myQuery->close();
